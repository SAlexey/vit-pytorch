...
class Dino(nn.Module):
    def __init__(
        ...
        self.local_crop = T.RandomResizedCrop(
            image_size, scale=(0.05, local_upper_crop_scale)
        )
        self.global_crop = T.RandomResizedCrop(
            image_size, scale=(global_lower_crop_scale, 1.0)
        )
        ...
        self.forward(torch.randn(2, 1, *image_size, device=device))
    ...
    def forward(
        ...
        local_image_one, local_image_two = (
            self.local_crop(image_one),
            self.local_crop(image_two),
        )
        global_image_one, global_image_two = (
            self.global_crop(image_one),
...
def build(net, args):
    return Dino(net, args.image_size,)
